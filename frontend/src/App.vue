<script setup lang="ts">
  ...
  signupForm.value.register(async (event: any) => {
...
<template>
  ...
  <div v-if="signupForm.step === 2">
    <div>
      <label @click="signupForm.populate()">Name:</label>
      <div>
    ...
    <div>
      <label>Email:</label>
      <div>
        <input id="input-email" type="email" v-model="signupForm.email" />
      </div>
    </div>
    <div>
      <label>Cpf:</label>
  ...
  <div v-if="signupForm.step === 4">
    <div v-if="accountId" id="account-id">
      {{ accountId }}
    </div> 
  </div>
  ...
  <button
    v-if="signupForm.isPreviousButtonVisible()"
    id="previous-button"
  ...
  <button 
    v-if="signupForm.isNextButtonVisible()"
    id="next-button"
  ...
  <button 
    v-if="signupForm.isSubmitButtonVisible()"
    id="submit-button"
